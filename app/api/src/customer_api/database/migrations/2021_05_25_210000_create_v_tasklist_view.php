...
class CreateVTasklistView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    private function createView(): string
    {
        return
            ...
            select
              ...
              0 as data_scope,
              expire.expire_count,
              contract_waiting.contract_waiting_count,
              renewal_waiting.renewal_waiting_count,
              renewal_this_month.renewal_this_month_count,
            ...
            from cu_customer cust
             ...
             left join (
                   select
                     ...
                     from cu_contract cc
                          inner join cu_project cp on cp.project_id = cc.project_id
                    where cc.quote_available_end_date BETWEEN DATE_ADD(CURRENT_DATE, INTERVAL 60 DAY) and DATE_ADD(CURRENT_DATE, INTERVAL 30 DAY)
             ...
             left join (
                   select
                     ...
                     from cu_contract cc
                          inner join cu_project cp on cp.project_id = cc.project_id
                    where cc.quote_available_end_date BETWEEN DATE_ADD(CURRENT_DATE, INTERVAL 30 DAY) and CURRENT_DATE
                      and cc.extension_type = 0
                     and cc.status
                     and cp.status
                  group by cp.customer_id
    ...
    {
        return "DROP VIEW IF EXISTS 'v_tasklist'";
    }
}
