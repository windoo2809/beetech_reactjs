...
class UpdateTrigger20220124V2 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {   
        return "
            ...
            -- 更新日：22/01/23
            DROP TRIGGER IF EXISTS upd_bf_cu_estimate;

            CREATE TRIGGER upd_bf_cu_estimate  BEFORE UPDATE ON cu_estimate FOR EACH ROW
            BEGIN

            -- 連絡コメント
            IF new.estimate_status = 2 AND old.estimate_status = 1 THEN
            
                SET new.contact_memo = 'システムへアップロードを完了しました（発注書を弊社で確認後、駐車場所有者様への最終確認を行います）';

            ELSEIF new.estimate_status = 7 AND old.estimate_status <> 7  THEN

                SET new.contact_memo = 'この見積はキャンセルとなりました';
